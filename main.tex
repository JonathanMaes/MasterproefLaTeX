...
\begin{figure} % TODO: Maybe this figure is quite ugly idk, maybe swap for an actual svg of ellipses
    \centering
...
% TODO: Can still include something on jump noise process and Curie temperature (phd_leliaert) and if I have time a bit more on solvers but not strictly necessary (start of appendix in phd_leliaert)
...
The geometry is the most important factor determining the energy barrier. Shown in \cref{fig:barrier-cell_size} is the dependence of the energy barrier on the roundness $\rho$, for different total sizes $L$. A positive value in this figure indicates that $E(\Theta=\pi/4) > E(\Theta=0)$, while for a negative value this is opposite. This means that the first-order energy barrier is responsible for the sign in the plot, while it is the full energy barrier that is plotted. \par
...
The global behavior as function of cell size, is that larger cells result in a lower absolute value of the energy barrier. The exception to this is for nearly round geometries ($\rho \approx 1$). In reality, there should not be a preferential direction for a round geometry. Because of the Cartesian simulation grid, however, a preferential orientation is created in the simulation. For large cells, this causes the energy barrier for a round geometry to become very large. \par
When looking closely at the region around $\rho=0.48$, it is observed that the energy barrier does not get as close to zero for the \SI{1}{\nano\metre} cells as it does for the other cell sizes. Thus, the higher-order energy barriers follow the same dependence on the cell size as the full energy barrier. \par % TODO: this explanation might need an inset figure with datapoints, and then a better explanation lol
Everything taken into account, it seems that the optimal compromise between simulation speed and accuracy, is a \SI{2}{\nano\metre} cell size. However, if one is solely interested in the physics corresponding to a specific value of the energy barrier, while making abstraction of the geometry, any cell size can be used as long as the corresponding energy barrier for that discretization is known.
...
%TODO: explain the thing with nanomagnetic directions before explaining these rules
...
Note that for the 'in 2 -> out 1' case, it does not really matter in which direction the 'out 1' points, as long as it is horizontal.
