...
%There are generally two ways one can go about designing a half adder. One way is to choose a geometry based upon a certain envisioned quaternary convention. The second way is to pick a geometry and then determine the quaternary convention(s) that can allow the geometry to work as a half adder. It is advisable to first choose a geometry following a certain train of thought, and when this geometry seems to work, one can vary some parameters to see if a better half adder is possible by slightly changing this geometry, thus combining the two ways. % TODO: this is quite a louche explanation idk
...
% TODO: Include something like "It is possible that, during the relaxation, the magnetization of an island rotates more than \SI{90}{\degree} to reach an energy minimum, which means that the initial combination of magnetization angles will not show up in the results. This is not a problem, as we are only interested in the physical states, and in such a case the initial combination was clearly not stable." in the part where I talk about the energy levels of the half adder
...
\end{figure} % TODO: maybe rename In 1 and In 2 to In L and In R.
...
% TODO: move this line to after the balancedness discussion:
...
% TODO: I think the $\max_\alpha(E_{\alpha,0}) - \min_\alpha(E_{\alpha,0})$ metric is interesting only after the $\min_\alpha(E_{\alpha,1}) - \max_\alpha(E_{\alpha,0})$ metric is fulfilled. So figure out which to call which and how to explain this best.
