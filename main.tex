...
% TODO: Perhaps make figure for (x x x) configuration of nanowire
One way to transmit biaxial information is to use a nanowire in a similar manner as in the uniaxial case, where the biaxial islands are placed diagonally such that all states are equally low in energy. % TODO: refer to the interaction section for a figure with two 45° rotated islands
...
\ctable[
    ...
    \tnote[a]{This energy barrier is comparable to the thermal energy $k_B T$. Hence, the switching rate ($\approx \SI{2}{\nano\second}$) is on the same timescale as the LLG dynamics, which causes the estimate for $f_0$ using \cref{eq:f0_theoretical_general} to be inaccurate.}
        ...
        $\rho$ & Grid [nm] & Barrier [meV] & T [K] & $\alpha$ & $\Delta t$ [ns] & $N$ & $f_0$ \\
        \hline
...
The dependence of $f_0$ on the damping constant $\alpha$, energy barrier and temperature was already examined qualitatively in the previous paragraphs. The parameters of these different simulations are listed in \cref{tab:Switching_f0}. For each simulation, a theoretical value for $f_0$ was estimated using \cref{eq:f0_experimental} as listed in the last column of the table. All the simulations for which the energy barrier is significantly higher than the thermal energy yield a value around $f_0=\SI{e10}{\per\second}$, which is of the same order of magnitude as the intrinsic resonance frequency of the small sinus of the energy minimum. % TODO: word this and the next line differently
