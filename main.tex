...
It is expected that there is an exponential dependency on the ratio between energy barrier and temperature. To investigate this, simulations spanning \SI{1}{\micro\second} were carried out for two different temperatures and two different energy barriers. The different energy barriers correspond to $\rho=0.65$ and $\rho=1$ geometries at a cell size of \SI{4}{\nano\metre}. These have energy barriers of \SI{155.0}{\milli\electronvolt} and \SI{241.1}{\milli\electronvolt}, respectively, with opposite easy and hard axes. This is clearly visible in the figures, as the \SI{155.0}{\milli\electronvolt} case is stable around $\SI{45}{\degree} + n\SI{90}{\degree}$, while \SI{241.1}{\milli\electronvolt} is stable around $n\SI{90}{\degree}$. The exponential dependence on the temperature is qualitatively clear from the difference between \cref{fig:switching-temp-300-65x100} and \ref{fig:switching-temp-350-65x100}, where the latter has a lot more switches for only a \SI{16}{\percent} increase in temperature. The exponential dependence on the energy barrier can be seen in the difference between \cref{fig:switching-temp-300-65x100} and \ref{fig:switching-temp-350-100x100}, where the latter has not a single switch due to the \SI{55}{\percent} higher energy barrier. 
\begin{figure}
     \centering
...
In the limit where the energy barrier is comparable to the thermal energy $k_B T$, the theoretical formulae above no longer apply. An example of a \SI{100}{\nano\second} simulation for an energy barrier of \SI{24.7}{\milli\electronvolt} is shown in \cref{fig:switching-49x100-300}. The switching frequency is very high in this case, and the magnetization direction seems to perform some sort of random walk.
\begin{figure}
    \centering
...
There exists some ambiguity as to how one can count the amount of switches during a time interval. Here, not every crossing of the magnetization direction through an energy maximum is counted as a switch; switches larger than \SI{90}{\degree} are counted as one single switch. As soon as the magnetization starts turning in the opposite direction, any subsequent \SI{90}{\degree} turn or higher is counted as another switch.
...
\ctable[
    cap = Switching Rates,
    caption = {Switching rates and corresponding $f_0$ values for different geometries and grid discretizations, all for $\alpha=0.01$.},
    ...
    \tnote[a]{The energy barrier in this situation is of similar size as the thermal energy $k_B T$, and hence the exponential law from \cref{eq:f0_experimental} no longer holds. For such low barrier, the switching rate ($\approx \SI{2}{\nano\second}$) is on the same timescale as the LLG dynamics. Hence the exponential relationship of the Arrhenius law is no longer entirely applicable, causing the corresponding values for $f_0$ to be inaccurate.}
    }{
...
Parameters that can affect $f_0$ according to formula:
- Alpha: doesnt seem to have a significant effect in the simulation, but should according to formula :l
- Msat: didnt change that in sim
- Volume: approximate
- Energy barrier: haha yes
- Temperature: haha yes
...
Care needs to be taken that each island is placed on the grid in the correct manner, i.e. that the energy barrier is the same for both. For example, when determining the energy as function of the distance between the islands, the energy will fluctuate wildly if the islands are not placed at integer cell sizes away from each other.
